...
def main():
    ...
    if ftype == "POSDATAMJ":
        ...
        keys = tuple(args.key.split(",")) if args.key else ("P0STR", "P0ISBN", "P0ACOD")
        ...
        keys = tuple(args.key.split(",")) if args.key else ("PSAGY", "PSCHN", "PSITM#", "PSWEDT")
        excl = set(args.exclude.split(",")) if args.exclude else set()
        result = validate_possumm(args.golden, args.generated, key_fields=keys, exclude_fields=excl)
    elif ftype == "POSDTLM":
        keys = tuple(args.key.split(",")) if args.key else ("PDAGY", "PDCHN", "PDSTR", "PDITM#", "PDWEDT")
        excl = set(args.exclude.split(",")) if args.exclude else set()
        result = validate_posdtlm(args.golden, args.generated, key_fields=keys, exclude_fields=excl)
    else:
