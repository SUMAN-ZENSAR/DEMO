...
// ---------------------------------------------------------------------------
function buildPOSDATAMJ(r, children, heading) {
  const pass = r.facts_missing_in_golden === 0 && r.facts_qty_mismatch === 0;
  if (heading) children.push(heading1(heading));
...
function buildPOSNLSN(r, children, heading) {
...
function buildPOSDLYM(r, children, heading) {
  ...
  if (r.excluded_fields && r.excluded_fields.length) {
    children.push(bullet(`Excluded from comparison (environment/run-specific, not a correctness signal): ${r.excluded_fields.join(", ")}`));
  }
...
// ---------------------------------------------------------------------------
function dispatchBuild(r, children, heading) {
  if (r.file_type === "POSDATAMJ") buildPOSDATAMJ(r, children, heading);
  else if (r.file_type === "POSNLSN") buildPOSNLSN(r, children, heading);
  else if (["POSDLYM", "POSSUMM", "POSDTLM"].includes(r.file_type)) buildPOSDLYM(r, children, heading);
...
function passOf(r) {
  ...
  const fr = r.field_results;
  return r.rows_missing_in_golden === 0 && Object.keys(fr).every(f => fr[f].diff === 0 && fr[f].soft === 0);
}
...
function main() {
  // Accepts one or more <results.json> files, then <output.docx>, then an optional
  // ["context line"]. One json => single-file report. Multiple => combined report.
  const args = process.argv.slice(2);
  let jsonPaths = [];
  let i = 0;
  for (; i < args.length; i++) {
    if (args[i].toLowerCase().endsWith(".json")) jsonPaths.push(args[i]);
    else break;
  }
  const outPath = args[i];
  const customTitle = args[i + 1];

  if (jsonPaths.length === 0 || !outPath) {
    console.error('Usage: node build_report.js <results1.json> [results2.json ...] <output.docx> ["context line"]');
    process.exit(1);
  }

  const resultsList = jsonPaths.map(p => JSON.parse(fs.readFileSync(p, "utf8")));
  const children = [];
  ...
  if (resultsList.length === 1) {
    ...
    children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 60 },
      children: [txt("COMBINED VALIDATION REPORT", { bold: true, size: 32, color: NAVY })] }));
    children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 },
      children: [txt(customTitle || resultsList.map(r => r.file_type).join(" + "), { bold: true, size: 21, color: BLUE })] }));
    children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 240 },
      children: [txt("Complete field-by-field check, no sampling, every mismatch listed however small", { italics: true, size: 18, color: "595959" })] }));
    const allPass = resultsList.every(passOf);
    children.push(calloutBox(allPass ? "OVERALL RESULT: ALL DATA MATCHED across every file validated." : "OVERALL RESULT: MISMATCH — see per-file sections below.", allPass ? "pass" : "fail"));
    children.push(heading1("Executive Summary"));
    children.push(table(["File", "Golden / Generated", "Verdict"], [4000, 3000, 2360],
      resultsList.map(r => [r.file_type, rowCountLabel(r), passOf(r) ? "MATCH" : "MISMATCH"]), 2));
    resultsList.forEach((r, idx) => {
      if (idx > 0) children.push(new Paragraph({ children: [], pageBreakBefore: true }));
      dispatchBuild(r, children, `${idx + 1}. ${r.file_type}`);
    });
  }
  ...
  const doc = new Document({
    sections: [{
      ...
      headers: { default: new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT,
        children: [txt(resultsList.length === 1 ? `${resultsList[0].file_type} Validation Report` : "Combined Validation Report", { size: 16, color: "808080" })] })] }) },
