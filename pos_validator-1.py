...
# Fields that are never compared for POSDLYM — environment/run-specific by
# nature, not a data-correctness signal. P0NOID is a sequence number from
# pos.POSDLYSEQN (expected to differ by environment/run). RunId only exists
# on the Generated side to begin with.
DEFAULT_EXCLUDE_FIELDS = {"P0NOID", "RunId"}
...
def validate_posdlym(golden_path, generated_path, key_fields=("P0STR", "P0ISBN", "P0ACOD"),
                      exclude_fields=None):
    exclude_fields = set(exclude_fields) if exclude_fields is not None else set(DEFAULT_EXCLUDE_FIELDS)
...
def main():
    ...
    ap.add_argument("--exclude", default="P0NOID,RunId",
                     help="Comma-separated fields to skip entirely in POSDLYM field-by-field comparison.")
    ...
    if ftype == "POSDATAMJ":
        ...
        result = validate_posdlym(args.golden, args.generated, key_fields=keys, exclude_fields=excl)
    else:
